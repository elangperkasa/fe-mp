...
export default function RoleBasedGuard({
  ...
  const currentTerritory = user?.UserTerritory;

  const check = useCallback(() => {
    // Handle new IdaMan users who don't have a role or position yet
    if (!currentRole || !currentPosition) {
      router.push('/403');
      return null;
    }
    ...
    if (!currentTerritory.length && currentRole === 'SAM') {
      router.push('/403?context=non-territory');
      return null;
    }
